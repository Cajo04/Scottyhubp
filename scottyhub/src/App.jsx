...
/* ═══════════════════════════════════════════════
   LOCAL CACHE — lightweight runtime store
   (news/media fetched from backend; no localStorage simulation)
═══════════════════════════════════════════════ */
const firebaseDB = {
...
const NAV = [
  ...
  { id: "crypto", icon: "💹", label: "Crypto & Forex" },
...
function BotsPage({ user, setPaymentItem }) {
  ...
  // ── Bot Status Live Ping ──
  const BOT_INSTANCES = [
    { name: "Scotty-C", url: BOT_PAIRING_URL },
    { name: "ScottyAI", url: "https://scottyai.onrender.com" },
  ];
  const [botStatus, setBotStatus] = useState(() => BOT_INSTANCES.map(b => ({ ...b, status: "checking" })));

  useEffect(() => {
    const checkAll = async () => {
      const results = await Promise.all(BOT_INSTANCES.map(async b => {
        try {
          const ctrl = new AbortController();
          const timeout = setTimeout(() => ctrl.abort(), 6000);
          const res = await fetch(`${b.url}/api`, { signal: ctrl.signal });
          clearTimeout(timeout);
          return { ...b, status: res.ok ? "online" : "degraded" };
        } catch {
          return { ...b, status: "offline" };
        }
      }));
      setBotStatus(results);
    };
    checkAll();
  ...
  return (
    <div style={{ animation: "fadeUp .5s ease" }}>
      ...
      <p style={{ color: C.muted, fontSize: 14, marginBottom: 16 }}>Pair your bot for free. Enter your number below — no coding needed.</p>

      {/* Bot Status Bar */}
      <div style={{ display: "flex", gap: 10, flexWrap: "wrap", marginBottom: 22 }}>
        {botStatus.map((b, i) => {
          const col = b.status === "online" ? "#00cc88" : b.status === "degraded" ? "#F0B90B" : b.status === "checking" ? C.muted : "#ff4444";
          const icon = b.status === "online" ? "🟢" : b.status === "degraded" ? "🟡" : b.status === "checking" ? "⏳" : "🔴";
          return (
            <div key={i} style={{ display: "flex", alignItems: "center", gap: 8, background: `${col}0f`, border: `1px solid ${col}33`, borderRadius: 8, padding: "8px 14px" }}>
              <span style={{ fontSize: 13 }}>{icon}</span>
              <span style={{ fontFamily: "'Rajdhani',sans-serif", fontWeight: 700, fontSize: 13, color: col }}>{b.name}</span>
              <span style={{ fontSize: 10, color: C.muted, fontFamily: "'Share Tech Mono',monospace", textTransform: "uppercase" }}>{b.status}</span>
            </div>
          );
        })}
      </div>
...
function CommunityPage({ user }) {
  ...
  const fetchPosts = useCallback(async (p = 1) => {
    try {
      const res = await fetch(`${API_URL}/posts?page=${p}`, {
        headers: { Authorization: `Bearer ${token}` }
      });
      const data = await res.json();
      if (Array.isArray(data)) {
        setMessages(prev => p === 1 ? data.reverse() : [...data.reverse(), ...prev]);
      }
  ...
  const send = async () => {
    ...
    try {
      const res = await fetch(`${API_URL}/posts`, {
        ...
        body: JSON.stringify({ content: `[#${category}] ${input.trim()}` })
      });
  ...
  const displayed = filter === "all" ? messages : messages.filter(m => m.content?.includes(`[#${filter}]`));
    ...
    <div style={{ animation: "fadeUp .5s ease" }}>
      ...
      <div className="card" style={{ height: 420, display: "flex", flexDirection: "column", overflow: "hidden", marginBottom: 12 }}>
        <div style={{ flex: 1, overflowY: "auto", padding: "12px", display: "flex", flexDirection: "column", gap: 8 }}>
          {loading && <div style={{ textAlign: "center", padding: "60px 20px", color: C.muted }}><div style={{ fontSize: 30, marginBottom: 10 }}>⏳</div><p>Loading messages...</p></div>}
          {!loading && displayed.length === 0 && <div style={{ textAlign: "center", padding: "60px 20px", color: C.muted }}><div style={{ fontSize: 40, marginBottom: 10 }}>💬</div><p>Be the first to post in this channel!</p></div>}
          {!loading && displayed.map((m, i) => {
            const isMe = m.author?._id === user.uid || m.author?.username === user.name;
            const authorName = m.author?.username || "Anonymous";
            const text = m.content?.replace(/^\[#\w+\]\s*/, "") || m.content;
            const cat = m.content?.match(/^\[#(\w+)\]/)?.[1] || "general";
            return (
              <div key={m._id || i} style={{ display: "flex", gap: 9, alignItems: "flex-start" }}>
                <div style={{ width: 32, height: 32, borderRadius: "50%", background: isMe ? "linear-gradient(135deg,#00ffcc,#0077ff)" : "linear-gradient(135deg,#334455,#1a2a3a)", display: "flex", alignItems: "center", justifyContent: "center", fontWeight: 700, fontSize: 12, color: isMe ? "#060b14" : C.text, flexShrink: 0 }}>{authorName[0].toUpperCase()}</div>
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 3, flexWrap: "wrap" }}>
                    <span style={{ fontWeight: 700, fontSize: 12, color: isMe ? C.cyan : C.text }}>{authorName}</span>
                    <span style={{ fontSize: 9, padding: "1px 6px", borderRadius: 2, background: "rgba(0,255,204,.06)", color: C.muted, fontFamily: "'Share Tech Mono',monospace" }}>#{cat}</span>
                    <span style={{ color: C.muted, fontSize: 10, fontFamily: "'Share Tech Mono',monospace" }}>{new Date(m.createdAt).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })}</span>
                    <span style={{ color: C.muted, fontSize: 10 }}>❤ {m.likes?.length || 0}</span>
                  </div>
                  <div style={{ fontSize: 13, lineHeight: 1.5, color: C.text, background: isMe ? "rgba(0,255,204,.06)" : "rgba(10,16,28,.9)", padding: "8px 12px", borderRadius: "0 8px 8px 8px", border: "1px solid rgba(0,255,204,.06)", display: "inline-block", maxWidth: "100%", wordBreak: "break-word" }}>{text}</div>
                </div>
              </div>
            );
          })}
        ...
        <div style={{ padding: "10px 12px", borderTop: "1px solid rgba(0,255,204,.08)" }}>
          ...
          <div style={{ display: "flex", gap: 8 }}>
            ...
            <button className="btn btn-cyan" onClick={send} disabled={posting} style={{ padding: "9px 16px", borderRadius: 8, opacity: posting ? 0.6 : 1 }}>{posting ? "..." : "Send"}</button>
          </div>
        </div>
      </div>
...
function LeaderboardPage({ user }) {
  ...
  useEffect(() => {
    // Fetch leaderboard data from posts (most active = most posts)
    const fetchBoard = async () => {
      try {
        const res = await fetch(`${API_URL}/posts?page=1`, {
          headers: { Authorization: `Bearer ${token}` }
        });
        ...
        if (Array.isArray(posts)) {
          const counts = {};
          posts.forEach(p => {
            const name = p.author?.username || "Anonymous";
            const id = p.author?._id || name;
            if (!counts[id]) counts[id] = { name, score: 0, id };
            counts[id].score++;
          });
          setBoard(Object.values(counts).sort((a, b) => b.score - a.score).slice(0, 10));
        }
      } catch (e) { console.error(e); }
      setLoading(false);
    };
  ...
  return (
    <div style={{ animation: "fadeUp .5s ease" }}>
      ...
      <p style={{ color: C.muted, fontSize: 14, marginBottom: 20 }}>Rankings based on community activity. Most active users rise to the top.</p>

      {loading ? (
        <div className="card" style={{ padding: "40px", textAlign: "center" }}>
          <div style={{ fontSize: 36, marginBottom: 12 }}>⏳</div>
          <p style={{ color: C.muted }}>Loading leaderboard...</p>
        </div>
      ) : board.length === 0 ? (
        <div className="card" style={{ padding: "40px", textAlign: "center" }}>
          <div style={{ fontSize: 48, marginBottom: 12 }}>🏆</div>
          <p style={{ color: C.muted }}>No activity yet. Be the first to post in Community!</p>
        </div>
      ) : (
        <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
          {board.map((u, i) => (
            <div key={i} className="card" style={{ padding: "14px 18px", display: "flex", alignItems: "center", gap: 12, border: u.name === user.name ? "1px solid rgba(0,255,204,.3)" : "1px solid transparent" }}>
              ...
              <div style={{ width: 36, height: 36, borderRadius: "50%", background: u.name === user.name ? "linear-gradient(135deg,#00ffcc,#0077ff)" : "linear-gradient(135deg,#334455,#1a2a3a)", display: "flex", alignItems: "center", justifyContent: "center", fontWeight: 700, fontSize: 14, color: u.name === user.name ? "#060b14" : C.text, flexShrink: 0 }}>{(u.name || "U")[0].toUpperCase()}</div>
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ fontWeight: 700, fontSize: 14, display: "flex", alignItems: "center", gap: 8 }}>
                  {u.name} {u.name === user.name && <span style={{ fontSize: 9, padding: "1px 6px", background: "rgba(0,255,204,.1)", color: C.cyan, borderRadius: 3, fontFamily: "'Share Tech Mono',monospace" }}>YOU</span>}
                </div>
                <div style={{ color: C.muted, fontSize: 11, fontFamily: "'Share Tech Mono',monospace" }}>{u.score} posts</div>
              </div>
              <div style={{ textAlign: "right" }}>
      ...
      <div style={{ marginTop: 20, padding: "14px 16px", background: "rgba(0,255,204,.04)", border: "1px solid rgba(0,255,204,.1)", borderRadius: 8 }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
          {[["Post in Community", "+1 pt/post"], ["Refer a new user", "+10 pts"], ["Referral upgrades to Pro", "+50 pts"], ["Referral upgrades to Business", "+100 pts"]].map(([a, p]) => (
